...
export const fetchCars = async (make: string, model: string, year: string, page:string): Promise<IFetchCarsReturn> => {
  let url =
    `${import.meta.env.VITE_API_URL}/all-vehicles-model/records?`;

    // eger marka filtresi varsa, istek atilan url'e ekle

    if(make) {
      url += `&refine=make:"${make}"`;
    }

    // eger model filtresi varsa, istek atilan url'e ekle
    if(model) {
      url += `&refine=model:"${model}"`;
    }
// eger year filtresi varsa, istek atilan url'e ekle
    if(year) {
